...
UIWindow *alertWindowOutUnderVideo;
...
- (void)tableView:(UITableView *)theTableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.section == 2) {
        ...
        if(indexPath.row == 1) {
            UIWindow *alertWindowUnderVideo = [[UIWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
            alertWindowUnderVideo.rootViewController = [UIViewController new];
            alertWindowUnderVideo.windowLevel = UIWindowLevelAlert + 1;
            alertWindowOutUnderVideo = alertWindowUnderVideo;
    
            UnderVideoOptionsController *underVideoOptionsController = [[UnderVideoOptionsController alloc] init];
            UIBarButtonItem *doneButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(undervideodone)];
            underVideoOptionsController.navigationItem.rightBarButtonItem = doneButton;
            ...
            [alertWindowUnderVideo makeKeyAndVisible];
            [alertWindowUnderVideo.rootViewController presentViewController:underVideoOptionsControllerView animated:YES completion:nil];
        }
        if(indexPath.row == 2) {
...
- (void)undervideodone {
    alertWindowOutUnderVideo.hidden = YES;
}
